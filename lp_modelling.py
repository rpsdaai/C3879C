...
#
# Do unit testing (Feature Engineering)
#
def do_test_with_fe(filename):
	data = [
		{
			'Gender': 'Male',
			'Married': 'Yes',
...
if __name__ == '__main__':
	log.debug('START Modelling ....')
	# oheFlag=True, scaleFlag=True, feFlag=True
	# X, y = lp_eda.do_processDataset(oheFlag=True, scaleFlag=True)
	# X, y = lp_eda.do_processDataset(oheFlag=True, scaleFlag=True, feFlag=True)
	...
	accuracyList, aucrocList, modelList = do_computeModelAccuracy(X, y)
	do_plotRocCurves(aucScores_, 'roc_curve_no_gs.png')
	...
	do_createModel(modelList[model2UseIndex], X, y, 'my_no_fe_gs_model_2.pkl', 'p')
	do_test_with_fe('my_no_fe_gs_model_2.pkl')

	classifierlist.clear()
	aucScores_.clear()
	aucScoresCounter = 0

	do_useGridSearch2FindBestModel(X, y)
	log.debug('Classifierlist Length: ' + str(len(classifierlist)))
	for i in range(len(classifierlist)):
		log.debug(classifierlist[i])


	do_plotRocCurves(aucScores_, 'roc_curve_gs.png')
	model = do_getModelScores_fromGridSearch()
	do_createModel(model, X, y, 'mygs_fe_model_2.pkl', 'p')
	# do_test('mygsmodel.pkl')
	do_test_with_fe('mygs_fe_model_2.pkl')
